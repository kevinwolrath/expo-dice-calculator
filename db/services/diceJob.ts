...
export const createDiceJob = async (input: {
  ...
  await db.runAsync(
    `INSERT INTO dice_job
      (dice_job_id, job_name, description, colour_count, colour_count_manual, material_type_id, material_type_manual, production_method_id, production_method_manual, dice_job_number_colour_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
    id,
    input.job_name,
    input.description ?? null,
    input.colour_count,
    input.colour_count_manual,
    input.material_type_id,
    input.material_type_manual,
    input.production_method_id,
    input.production_method_manual,
...
export const updateDiceJob = async (
  ...
  await db.runAsync(
    `UPDATE dice_job
     SET job_name = ?, description = ?, colour_count = ?, colour_count_manual = ?, material_type_id = ?,
       material_type_manual = ?, production_method_id = ?, production_method_manual = ?,
       dice_job_number_colour_id = ?, updated_at = CURRENT_TIMESTAMP
     WHERE dice_job_id = ?;`,
    input.job_name ?? current.job_name,
    input.description !== undefined ? input.description : current.description,
    input.colour_count ?? current.colour_count,
    input.colour_count_manual ?? current.colour_count_manual,
    input.material_type_id ?? current.material_type_id,
    input.material_type_manual ?? current.material_type_manual,
    input.production_method_id ?? current.production_method_id,
    input.production_method_manual ?? current.production_method_manual,
